...
public class Config {
    ...
    private static final String DEFAULT_CONFIG = "src/test/java/org/hyperledger/fabric/mysdkintegeration/utils.properties";
    ...
    private final static HashMap<String, MySampleOrg> sampleOrgs = new HashMap<>();
...
//            defaultProperty(INTEGRATIONTESTS_ORG + "peerOrg1.peer_locations", "peer0@grpc://localhost:7051");
            ...
            for (Map.Entry<Object, Object> x : sdkProperties.entrySet()) {
                ...
                if (key.startsWith(INTEGRATIONTESTS_ORG)) {
                    ...
                    if (match.matches() && match.groupCount() == 1) {
                        ...
                        sampleOrgs.put(orgName, new MySampleOrg(orgName, val.trim()));

                    }
                }
            }

            for (Map.Entry<String, MySampleOrg> org : sampleOrgs.entrySet()) {
                final MySampleOrg mySampleOrg = org.getValue();
                ...
                for (String peer : ps) {
                    ...
                    mySampleOrg.addPeerLocation(nl[0], grpcTLSify(nl[1]));
                }
                ...
                for (String peer : ps) {
                    String[] nl = peer.split("[ \t]*@[ \t]*");
                    mySampleOrg.addOrdererLocation(nl[0], grpcTLSify(nl[1]));
                }
                ...
                for (String peer : ps) {
                    String[] nl = peer.split("[ \t]*@[ \t]*");
                    mySampleOrg.addEventHubLocation(nl[0], grpcTLSify(nl[1]));
                }


                mySampleOrg.setCALocation(httpTLSify(sdkProperties.getProperty((INTEGRATIONTESTS_ORG + org.getKey() + ".ca_location"))));
                if(runningTLS){
                    String cert = tlsbase + "/cas/" + mySampleOrg.getName() + "/cert.pem";
                    ...
                    mySampleOrg.setCAProperties( properties);
                }
            }
    ...
    public Collection<MySampleOrg> getIntegrationTestsSampleOrgs() {
        return Collections.unmodifiableCollection(sampleOrgs.values());
    }

    public MySampleOrg getIntegrationTestsSampleOrg(String name) {
